...
export class Container {
    ...
    constructor(entry?: PageEntry) {
        ...
        ($.fn as any).datebox.defaults.formatter = function(date) {
        ...
        ($.fn as any).datetimebox.defaults.formatter = function(date) {
            let y = date.getFullYear();
            let m = date.getMonth()+1;
            let d = date.getDate();
        ...
        ($.fn as any).datetimebox.defaults.parser = function(s) {
            let t = Date.parse(s);
            if (!isNaN(t)) {
                return new Date(t);
            } else {
